...
func TestKeyDerive_gnunet(t *testing.T) {
	var (
		pub = []byte{
		...
		query = []byte{
			0x4c, 0x3a, 0x31, 0xa5, 0xf5, 0x84, 0x77, 0x8d,
			0xdf, 0x38, 0x21, 0x8c, 0xa5, 0xae, 0xc1, 0x78,
			0x66, 0x90, 0x48, 0x80, 0xc8, 0x43, 0x27, 0xcc,
			0x66, 0xbd, 0xec, 0x57, 0x08, 0x11, 0xa6, 0xd0,
			0x8c, 0x03, 0x36, 0x96, 0x6d, 0x28, 0x66, 0x00,
			0xae, 0x34, 0x84, 0xb6, 0x11, 0xb8, 0x77, 0x7a,
			0xca, 0x63, 0xa0, 0x1e, 0x9f, 0xf7, 0xf9, 0x2c,
			0xb2, 0xc4, 0x88, 0x80, 0x6b, 0x13, 0xe6, 0x9f,
		}
	)

	pkey := NewPublicKeyFromBytes(pub)
	label := "master-home"
	if testing.Verbose() {
		t.Logf("H = %s\n", hex.EncodeToString(pkey.DeriveH(label, "gns").Bytes()))
	}
	pk2 := pkey.DeriveKey(label, "gns")
	out := sha512.Sum512(pk2.Bytes())
